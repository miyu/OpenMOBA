...
#else
using IntPoint = Dargon.PlayOn.Geometry.IntVector2;
#endif
#endif

namespace Dargon.PlayOn.ThirdParty.ClipperLib {
  ...
  using cDouble = System.Double;
   ...
   public class ClipperOffset {
      ...
      internal static DoublePoint GetUnitNormal(IntPoint pt1, IntPoint pt2) {
         ...
         cDouble f = cDouble_1 / CDoubleMath.Sqrt(dx * dx + dy * dy);
         dx *= f;
         dy *= f;
      ...
      private void DoOffset(cDouble delta) {
         ...
         if (ArcTolerance <= cDouble_0)
            y = def_arc_tolerance;
         else if (ArcTolerance > CDoubleMath.Abs(delta) * def_arc_tolerance)
            y = CDoubleMath.Abs(delta) * def_arc_tolerance;
         else
            y = ArcTolerance;
         //see offset_triginometry2.svg in the documentation folder ...
         cDouble steps = cDouble_PI / CDoubleMath.Acos(cDouble_1 - y / CDoubleMath.Abs(delta));
         m_sin = CDoubleMath.Sin(two_pi / steps);
         m_cos = CDoubleMath.Cos(two_pi / steps);
      ...
      void OffsetPoint(int j, ref int k, JoinType jointype) {
         ...
         if (CDoubleMath.Abs(m_sinA * m_delta) < cDouble_1) {
            //dot product ...
      ...
      internal void DoSquare(int j, int k) {
         cDouble dx = CDoubleMath.Tan(CDoubleMath.Atan2(m_sinA,
      ...
      internal void DoRound(int j, int k) {
         cDouble a = CDoubleMath.Atan2(m_sinA,
            m_normals[k].X * m_normals[j].X + m_normals[k].Y * m_normals[j].Y);
         int steps = Math.Max((int)Round(m_StepsPerRad * CDoubleMath.Abs(a)), 1);
