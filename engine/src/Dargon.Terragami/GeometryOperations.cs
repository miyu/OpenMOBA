...
namespace Dargon.PlayOn.Geometry {
   public static class GeometryOperations {
      ...
      // todo: epsilon tolerance is poor
      ...
      public static bool TryFindNonoverlappingLineLineIntersectionEx(in IntLineSegment2 a, in IntLineSegment2 b, out cDouble tForA, out cDouble tForB) {
         // via http://stackoverflow.com/questions/563198/how-do-you-detect-where-two-line-segments-intersect
         var p = a.First;
      ...
      public static bool TryFindNonoverlappingLineLineIntersectionEx(in DoubleLineSegment2 a, in DoubleLineSegment2 b, out cDouble tForA, out cDouble tForB) {
         // via http://stackoverflow.com/questions/563198/how-do-you-detect-where-two-line-segments-intersect
         var p = a.First;
         var r = a.First.To(a.Second);
         var q = b.First;
      ...
      public static bool TryFindNonoverlappingSegmentLineIntersectionT(in IntLineSegment2 segment, in IntLineSegment2 line, out cDouble tForSegment) {
         // via http://stackoverflow.com/questions/563198/how-do-you-detect-where-two-line-segments-intersect
         var p = line.First;
         var r = line.First.To(line.Second);
         var q = segment.First;
         ...
         var qmp = q - p;
         //var t = (cDouble)Cross(qmp, s) / (cDouble)rxs; //  tforLine
      ...
      public static bool TryFindNonoverlappingSegmentLineIntersectionT(in DoubleLineSegment2 segment, in DoubleLineSegment2 line, out cDouble tForSegment) {
         // via http://stackoverflow.com/questions/563198/how-do-you-detect-where-two-line-segments-intersect
         var p = line.First;
         var r = line.First.To(line.Second);
         var q = segment.First;
      ...
      public static bool ConvexPolygonContainsPoint(IntVector2 x, IntVector2[] p) {
#if DEBUG
         if (Clockness(p[0], p[1], p[2]) == Clk.CounterClockWise) throw new BadInputException("p not ccw");
         ...
         for (var i = 0; i < p.Length; i++) {
            ...
            var b = p[i];
            if (Clockness(a, b, x) == Clk.CounterClockWise) return false;
         }
         return true;
      }
      ...
      public static bool SegmentIntersectsNonDegenerateConvexPolygonInterior(IntLineSegment2 s, IntVector2[] p) {
         ...
         for (; i < p.Length && (xInterior || yInterior); i++, a = b) {
            ...
            if (abx == Clk.CounterClockWise && aby == Clk.CounterClockWise) return false;
            xInterior &= abx != Clk.CounterClockWise;
            yInterior &= aby != Clk.CounterClockWise;
...
throw new NotImplementedException();
      ...
      public static unsafe IntVector2[] ConvexHull(IntVector2[] input) {
         ...
         for (var i = 0; i < input.Length; i++) {
            while (lowerLength >= 2 && Clockness(lower[lowerLength - 2], lower[lowerLength - 1], input[i]) != Geometry.Clockness.ClockWise) {
               lowerLength--;
            }
         ...
         for (var i = input.Length - 1; i >= 0; i--) {
            while (upperLength >= 2 && Clockness(upper[upperLength - 2], upper[upperLength - 1], input[i]) != Geometry.Clockness.ClockWise) {
               upperLength--;
            }
      ...
      public static IntVector2[] ConvexHull4(IntVector2 a, IntVector2 b, IntVector2 c, IntVector2 d) {
         ...
         // make abc ccw
         if (abc == Clk.CounterClockWise) (a, c) = (c, a);
         ...
         if (abd == Clk.ClockWise) {
            if (bcd == Clk.ClockWise && cad == Clk.ClockWise) return new[] { a, b, c };
            if (bcd == Clk.ClockWise && cad == Clk.CounterClockWise) return new[] { a, b, c, d };
            if (bcd == Clk.CounterClockWise && cad == Clk.ClockWise) return new[] { a, b, d, c };
            if (bcd == Clk.CounterClockWise && cad == Clk.CounterClockWise) return new[] { a, b, d };
            throw new InvalidStateException();
         } else {
            if (bcd == Clk.ClockWise && cad == Clk.ClockWise) return new[] { a, d, b, c };
            if (bcd == Clk.ClockWise && cad == Clk.CounterClockWise) return new[] { d, b, c };
            if (bcd == Clk.CounterClockWise && cad == Clk.ClockWise) return new[] { a, d, c };
