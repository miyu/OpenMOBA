...
float2 findRayAndCenteredSphereIntersections(float3 rayOrigin, float3 rayDirection, float sphereRadius) {
    ...
    float twoA = 2.0 * a;
    return float2((-b - rootDiscriminant) / twoA, (-b + rootDiscriminant) / twoA);
}